...
def get_lang_step_text() -> str:
    return (
        "👋 *KOMEK DAMU*\n"
        "🤖 *Автоматический чат-бот / Автоматты чат-бот*\n\n"
        "🌐 *Шаг 1 — язык / Тіл:*\n\n"
        "1 — Қазақша\n"
        "2 — Русский\n\n"
        "✍️ Напишите *только цифру* *1* или *2* (не слово «Қазақша»)."
    )
...
def get_city_step_help(lang: str) -> str:
    """0 на шаге города — не зацикливать, объяснить что дальше будет список 1–7."""
    if lang == "kk":
        return (
            "📍 *Алдымен қалады таңдаңыз (2-қадам)*\n\n"
            "Содан кейін бөлімдер тізімі *1–7* ашылады.\n\n"
            f"{get_city_step_text(lang)}"
        )
    return (
        "📍 *Сначала выберите город (шаг 2)*\n\n"
        "После этого откроется список разделов *1–7*.\n\n"
        f"{get_city_step_text(lang)}"
    )
