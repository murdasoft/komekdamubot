...
from app.bot.ux_labels import BACK_LIST_KK, BACK_LIST_RU
...
WA_NAV_HINT_CITY = (
    f"0 — Разделы (1–7) / Бөлімдер\n"
    "7 — Менеджер и телефон / Менеджер\n"
    "98 — Список городов 1–5 / Қалалар тізімі\n"
    "99 — Сменить язык / Тілді ауыстыру"
)
...
HANDOFF_RELEASED_RU = "Возвращаю бота в диалог. Напишите /start — покажу разделы."
HANDOFF_RELEASED_KK = "Ботты диалогқа қайтарып аламын. Бөлімдер үшін /start жазыңыз."

# Error / unknown message
def get_unknown_message_with_phone(lang: str, city: str | None = None) -> str:
    phone = get_city_phone(city)
    if lang == "kk":
        return (f"Кешіріңіз, сұрауыңызды толық түсінбедім.\n\nСіз:\n• Қоңырау шала аласыз: {phone}\n• Бөлімдерге оралу: /start")
    return (f"Извините, не совсем понял ваш запрос.\n\nВы можете:\n• Позвонить: {phone}\n• Написать /start — список разделов")
...
Вместо этого бот показывает *список разделов с цифрами*:

```
📋 Разделы KOMEK DAMU

Напишите цифру нужного раздела:
...
Оның орнына бот *сандармен бөлімдер тізімін* көрсетеді:

```
📋 KOMEK DAMU бөлімдері
...
WA_MORTGAGE_MENU_KK = (
    ...
    "0️⃣ Бөлімдерге қайту\n\n"
    "Кеңес тек офисте. Санды жазыңыз:"
)

WA_INTRO_RU = (
    "📱 *WhatsApp режим*\n\n"
    "В WhatsApp — список разделов с цифрами:\n\n"
)

WA_INTRO_KK = (
    "📱 *WhatsApp режим*\n\n"
    "WhatsApp бөлімдер тізімін қолданады:\n\n"
)
