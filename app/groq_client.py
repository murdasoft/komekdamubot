...
def get_system_prompt(lang: str = "ru", city: str | None = None) -> str:
    ...
    return (
        ...
        "ПРИМЕРЫ:\n"
        f"— ИП кредит (клиент не спрашивал %): 'ИП-ке кредит береміз, 35 млн-ға дейін, 3 жылға. Ашық просрочка болмауы керек.\n{office_block} [DONE]'\n"
        f"— Физлицо (клиент не спрашивал %): 'Несие береміз. Пенсионкамен 15-20 млн, пенсионкасыз 8 млн-ға дейін.\n{office_block} [DONE]'\n"
        f"— Клиент спросил ставку ИП: 'ИП-ке ставка 21%-дан бастап, 35 млн-ға дейін, 3 жылға.\n{office_block} [DONE]'\n"
        f"— Ипотека: 'Ипотека бойынша кеңес тек офисте. Келіңіз.\n{office_block} [DONE]'\n"
        f"— Просрочки: 'Ашық просрочкамен болмайды. Жабыңыз да, келіңіз.\n{office_block} [DONE]'\n\n"
