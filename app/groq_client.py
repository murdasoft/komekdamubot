...
def get_system_prompt(lang: str = "ru", city: str | None = None) -> str:
    ...
    if lang == "kk":
        examples = (
            ...
            "— Чёрный список: 'Қара тізім жоқ. Тек несие тарихы бұзылады — бұл жөнделеді. Бұл үшін сіз консультацияға келуіңіз керек.\\n{office} [DONE]'\n"
            ...
            "— Несие жеке тұлғаға: 'Несие береміз. Пенсионкамен 25 млн-ға дейін, пенсионкасыз 25 млн-ға дейін, от 21%, 5 жыл. Офиске келіңіз.\\n{office} [DONE]'\n"
            "— ИП залог: 'ИП-ге кепілдік несие 40 млн-ға дейін, от 21%, 10 жылға. Кепіл: коммерция, үй, пәтер (авто/спецтехника ДАМУ-да емес). Без кепілдик ИП-ге DAMU жоқ. Келіңіз.\\n{office} [DONE]'\n"
            ...
            "БАЗА:\n"
            "— Ипотека: 2-9%, алғашқы жарна 10-20%, 25 жыл, 50 млн-ға дейін\n"
            "— ИП залог (ДАМУ): 40 млн, от 21%, 10 жыл. Кепіл: коммерция, үй, пәтер. Авто/спецтехника емес.\n"
            "— ТОО (ДАМУ): 200 млн, 12.6%, 1 жыл жұмыс\n"
            "— Жеке тұлға: 25 млн (пенсионкамен/без), от 21%, 5 жыл\n"
        ...
        examples = (
            ...
            "— Чёрный список: 'Чёрного списка нет. Просто портится кредитная история — это можно улучшить. Для этого вы должны прийти на консультацию.\\n{office} [DONE]'\n"
            "— Ипотека: 'Поможем с ипотекой. Гос.программа от 2%, взнос от 10-20%. Онлайн-консультации не даём — приходите в офис.\\n{office} [DONE]'\n"
            "— Физлицо: 'Кредит оформим. До 25 млн, от 21%, до 5 лет. Ждём вас.\\n{office} [DONE]'\n"
            "— ИП залог: 'Залоговый кредит для ИП до 40 млн, от 21%, до 10 лет. Залог: коммерция, дом, квартира (не авто/спецтехника). Беззалоговый DAMU для ИП нет. Приходите.\\n{office} [DONE]'\n"
            ...
            "БАЗА ЗНАНИЙ:\n"
            "— Ипотека: от 2-9%, первоначальный взнос 10-20%, до 25 лет, до 50 млн\n"
            "— ИП залоговый (ДАМУ): до 40 млн, от 21%, до 10 лет. Залог: коммерция, дом, квартира (не авто/спецтехника)\n"
            "— ТОО (ДАМУ): до 200 млн, 12.6%, от 1 года работы\n"
            "— Физлицо: до 25 млн, от 21%, до 5 лет\n"
