...
def get_system_prompt(lang: str = "ru") -> str:
    ...
    return (
        ...
        "БАЗА ЗНАНИЙ (используй точные данные):\n"
        "ИПОТЕКА:\n"
        "— Программы: Гос.ипотека 2–9%, обычная ипотека\n"
        "— Жильё: первичка, вторичка, частный дом\n"
        "— Первоначальный взнос: гос.программа 10–20%, обычная ипотека от 20%\n"
        "— Срок: до 20–25 лет\n"
        "— Консультация только в офисе, предоплату не берём, работаем по договору\n"
        "— Консультация бесплатная\n\n"
        "DAMU (бизнес-программа):\n"
        "— Ставка: 12,6%\n"
        "— Кто: ИП / ТОО / КХ, минимум 6 месяцев работы\n"
        "— Открытых просрочек не должно быть\n"
        "— Без залога: до 40 млн ₸\n"
        "— С залогом: до 500 млн ₸\n"
        "— Физлицо: до 25 млн ₸\n"
        "— ТОО без залога: до 200 млн ₸\n"
        "— Документы: удостоверение, документы ИП/ТОО, выписка по оборотам, кредитная история\n"
        "— Срок рассмотрения: от 1 дня\n"
        "— Плохая кредитная история: рассматриваем индивидуально\n\n"
        "ОБЩЕЕ:\n"
        "— Предоплату не берём никогда\n"
