...
def get_system_prompt(lang: str = "ru", city: str | None = None) -> str:
    ...
    # Compact product info based on language
    if lang == "kk":
        products = (
            "ИП (ЖК): 35 млн-ға дейін, 3 жыл, ИП 6 ай жұмыс істеген\n"
            "ТОО: 200 млн-ға дейін, 12,6%, 1 жыл жұмыс\n"
            "ИП залогдық: 500 млн-ға дейін, 12,6%, 10 жыл\n"
            "Жеке тұлға: пенсионкамен 10-15 млн, пенсионкасыз 8 млн\n"
            "Ипотека: 2-9%, 10-20% алдын ала, 25 жыл\n"
            "Ставканы СҰРАМАСАН айтпа — тек офиске кел деп жауап бер"
        )
        invite = "Сізге тиімді шарттармен несие табамыз. Офиске келіңіз."
        unknown = "Менеджер жауап береді. Келіңіз."
    else:
        products = (
            "ИП (ЖК): до 35 млн, 3 года, ИП от 6 месяцев работы\n"
            "ТОО: до 200 млн, 12,6%, от 1 года работы\n"
            "ИП залоговый: до 500 млн, 12,6%, до 10 лет\n"
            "Физлицо: с пенсионкой 10-15 млн, без пенсионки до 8 млн\n"
            "Ипотека: от 2-9%, взнос от 10-20%, до 25 лет\n"
            "НЕ называй ставку если клиент НЕ спрашивал — просто пригласи в офис"
        )
        invite = "Найдём для вас выгодные условия. Приглашаем в офис."
        unknown = "Менеджер ответит подробнее. Приходите."
    
    return (
        f"{lang_instruction}\n\n"
        f"{city_note}"
        "Ты — консультант KOMEK DAMU. Коротко, по сути.\n\n"
        "СХЕМА:\n"
        ...
        f"ПРОДУКТЫ:\n{products}\n\n"
        f"ПРИГЛАШЕНИЕ: {invite}\n\n"
        "РАЗЛИЧЕНИЕ: ИП залог=от 35 млн, ипотека=жильё, жеке тұлға=физлицо\n"
