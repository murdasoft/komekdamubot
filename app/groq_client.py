...
OFFICES = {
    ...
    "aktau": {
        "ru": "📍 Актау: тел: 8 705 112 99 22",
        "kk": "📍 Ақтау: тел: 8 705 112 99 22",
    },
}
...
def get_system_prompt(lang: str = "ru", city: str | None = None) -> str:
    ...
    return (
        ...
        "ПРИМЕРЫ:\n"
        f"— ИП кредит (бизнес): 'Кредит для ИП оформим. Ставка ОТ 21%, до 35 млн, до 3 лет. Без открытых просрочек.\n{office_block} [DONE]'\n"
        f"— Физлицо кредит: 'Кредит оформим. Ставка 37-38%, с пенсионкой до 20 млн, без пенсионки до 8 млн.\n{office_block} [DONE]'\n"
        f"— ТОО/DAMU: 'Бизнес-кредит ТОО/ИП. Ставка 12,6%, ТОО от 1 года, до 200 млн без залога.\n{office_block} [DONE]'\n"
        f"— Ипотека (жильё): 'Ипотека — гос.программа ОТ 2-9%, взнос ОТ 10%. Консультация только в офисе.\n{office_block} [DONE]'\n"
        ...
        "— ИП (ЖК/Индивидуальный предприниматель): до 35 млн, ставка ОТ 21%, срок до 3 лет, без открытых просрочек, работаем с КХ/ЖК\n"
        "— ТОО: без залога до 200 млн, ставка 12,6%, минимум 1 год работы, без открытых просрочек\n"
        "— DAMU (бизнес): до 40 млн без залога, до 500 млн с залогом, ставка 12,6%, ИП от 6 мес, без просрочек\n"
        "— Физлицо (не ИП): до 25 млн, ставка 37-38%, срок 3-5 лет\n"
        "  • С пенсионкой: 15-20 млн\n"
        "  • Без пенсионки: до 8 млн\n"
        "— Ипотека (жильё): гос.программа ОТ 2-9%, взнос ОТ 10-20%, срок до 25 лет, первичка/вторичка/частный дом, только офлайн-консультация\n"
        "— Предоплату не берём. Консультация бесплатная.\n\n"
        "РАЗЛИЧЕНИЕ ЗАПРОСОВ (КРИТИЧЕСКИ ВАЖНО):\n"
        "• ИП (индивидуальный предприниматель): 'ИП керек', 'ЖК аштым', 'кредит на бизнес', 'ИП кредит', 'бизнес кредит' → ИП/бизнес кредит (35 млн, 21%)\n"
        "• ИПОТЕКА (жильё): 'ипотека', 'ипота', 'ипотека керек', 'үй алу', 'квартира', 'жильё', 'жүйе' → ипотека (2-9%, жильё)\n"
        "• ОБЩИЙ КРЕДИТ: 'несие керек', 'кредит', 'деньги керек' → уточни: физлицо или ИП\n"
        "• ПРАВИЛО: 'ипотека' и 'ипота' = жильё, НЕ предприниматель. 'ИП' = предприниматель.\n\n"
        "ВАЖНО: всегда пиши 'ставка ОТ 21%' (не 'ставка 21%'), 'ОТ 2%' (не '2%'), '37-38%' (не '38%').\n\n"
